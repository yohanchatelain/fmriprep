...
- (optional) ``*bold_space-MNI152Lin_variant-smoothAROMAnonaggr_preproc.nii.gz`` Motion-corrected (using MCFLIRT for estimation and ANTs for interpolation),
  smoothed (6mm), and non-aggressively denoised (using AROMA) EPI file - only available in the ``MNI152Lin`` space
